...
const RegionalCascade = props => {
      ...
      cascadeLevels = [
        {
          ...
          compKey: 'province',
          api: 'https://www.fastmock.site/mock/29adb8c7e763fd69d52f9c23f533f21e/test/province/list',
        },
        {
          label: '请选择城市',
          labelEn: '',
          ...
          compKey: 'city',
          api: 'https://www.fastmock.site/mock/29adb8c7e763fd69d52f9c23f533f21e/test/city/list',
        },
        {
          label: '请选择行政区',
          labelEn: '',
          ...
          api:
            'https://www.fastmock.site/mock/29adb8c7e763fd69d52f9c23f533f21e/test/administrative/list',
        },
        {
          label: '请选择物业点',
          labelEn: '',
          ...
          compKey: 'property',
          api: 'https://www.fastmock.site/mock/29adb8c7e763fd69d52f9c23f533f21e/test/property/list',
        },
      ],
      compKey = '',
      parmasFilterFunc = '',
  ...
  useEffect(() => {
    if (cascadeLevels.length) {
      ...
      const fetchData = async () => {
        ...
        for (let i = 0; i < cascadeLevels.length; i++) {
          ...
          const apiParams = parmasFilterFunc
            ? filterDataFunc(parmasFilterFunc, { parentInfo: lastItem, ...item, itemKey: key })
            : { level: lastItem.level + 1, parentId: lastItem.id };

          newLoadings[index] = true;
          setLoadings(newLoadings);
          if (method === 'get') {
            res = await API[method](api, { params: apiParams, headers });
          }
          if (method === 'post') {
            res = await API[method](api, apiParams, { headers });
          }
          ...
          lastItem = resultList.find(
            item =>
              (name && item[nameKey] === name) || (value && `${item[valueKey]}` === `${value}`),
          );
      ...
      fetchData().then(res => {
        ...
        console.log('levelsData: ', newLevelsData);
        console.log('onChange: ', { [compKey]: obj });
  ...
  const onSelectChange = useCallback(
    async (value, dataSource, index, nameKey) => {
      const newLoadings = [...loadings];

      const newTotalData = { ...totalData };
      const currentValue = dataSource.find(n => n[nameKey] === value) || {};
      ...
      const nextApi = reap(cascadeLevels[index + 1], 'api');
      const params = parmasFilterFunc
        ? filterDataFunc(parmasFilterFunc, currentValue)
      ...
      if (nextApi && !hasData) {
        ...
        if (method === 'get') {
          setLoadings(newLoadings);
          res = await API.get(nextApi, { params, headers });
        ...
        if (method === 'post') {
          setLoadings(newLoadings);
          res = await API.post(nextApi, params, { headers });
          newLoadings[index + 1] = false;
          setLoadings(newLoadings);
        }
      ...
      {cascadeLevels.map((d, index) => {
        ...
        return (
          <Select
            ...
            onChange={value => onSelectChange(value, selectList, index, nameKey)}
            key={index}
            value={(levels[index] || {})[nameKey]}
