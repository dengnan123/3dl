...
export default class BarCom extends Component {
  ...
  getOptsAndSetOpts() {
    ...
    console.log('getOptsAndSetOpts opts', opts);
    echarts_instance.setOption(opts, { notMerge: true });
  }

  componentDidUpdate(prevProps) {
    const { doTimer, timer } = this.state;
    const { style: option = {}, data } = this.props;
  ...
  shouldComponentUpdate(nextProps, nextState) {
    const { data = {}, style: option = {} } = nextProps;
    const { series, categories } = data;
    const { series: preSeries, categories: preCategories, option: preOption } = this.state;
    if (
      isEqual(option, preOption) &&
      isEqual(preSeries, series) &&
      isEqual(categories, preCategories)
    ) {
      return false;
    }
    return true;
  ...
  render() {
    ...
    const options =
      option.newCode && option.copyEdit
        ? {
            ...option.newCode,
            legend: legend,
            series: data?.series || [],
            xAxis: [
              {
                ...option.newCode.xAxis[0],
                data: data?.categories || [],
              },
            ],
          }
        : this.getOption({ series, categories, type, option });
