...
import { Vector2, Vector3, Vector4, ShaderMaterial } from 'three';
...
class MultiplyLevelFlowPPS extends BasePPS {
  ...
  initSelf(data, obj) {
    ...
    data['animateParam']['t2'] = 57;
    data['animateParam']['t2Forward'] = true;
    initMainPass();
    ...
    function initGUI() {
      ...
      function initLevel() {
        ...
        function initGUI() {
          ...
          var globalFolder = gui.addFolder({
            folderName: '全局变量',
            key: 'Global层',
            isOpen: true,
          });
          ...
          var smoothstepData = {
            uniform: mainU['uvSmoothStepRange'],
            infos: [
              { displayName: '平滑偏置', isSlider: true, delta: 0.0001 },
              { displayName: '平滑长度', isSlider: true, delta: 0.0001 },
            ],
          };
          gui.addValue(smoothstepData, globalFolder);

          var stepFadeWidthData = {
            uniform: mainU['stepFadeWidth'],
            infos: [{ displayName: '层渐变宽度', isSlider: true, max: 2 }],
          };
          ...
          function initStep() {
            ...
            for (let index = 0; index < 10; index++) {
              var folder = gui.addFolder({
                folderName: `第${index + 1}层`,
                parent: mainFolder,
                isHide: index >= levelSettings.length,
              });
              sonFolders.push(folder);
              var data = {
                uniform: { value: settingStore[index] },
                infos: [
              ...
              var dataUV = {
                uniform: { value: uvSettingStore[index] },
                infos: [
            ...
            var numData = {
              uniform: { value: levelSettings.length },
              infos: [{ displayName: '层数量', isSlider: true, min: 1, max: 10, delta: 1 }],
              func: [onNumChange],
            };
  ...
  animateSelf(data, obj, deltaTime, isLog) {
    isLog && console.log("MultiplyLevelFlow");
    ...
    if (t1 >= 119) data['animateParam']['t1Forward'] = false;
    if (t1 <= 0) data['animateParam']['t1Forward'] = true;
    data['animateParam']['t1'] = t1;

    var t2 = data['animateParam']['t2'];
    // t2 += data['animateParam']['t2Forward'] ? 1 : -1;
    t2 += data['animateParam']['t2Forward'] ? 1 / 30 : -1 / 30;
    if (t2 >= 119) data['animateParam']['t2Forward'] = false;
    if (t2 <= 0) data['animateParam']['t2Forward'] = true;
    data['animateParam']['t2'] = t2;

    var offset = t1 / 120 * 0.55;
    var length = t2 / 120 * 0.24 + 0.24;
    // mainU.uvSmoothStepRange.value.x = offset;
    mainU.uvSmoothStepRange.value.set(offset, length);
    // obj.gui.refreshGUI();
  }
}

export { MultiplyLevelFlowPPS };
