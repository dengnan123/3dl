...
const ListContent = props => {
  ...
  const handleOk = useCallback(() => {
    // 可能初始化数据id不在列表中，存在脏数据，需要去除
    const finalKeys = keys.reduce((total, current) => {
      const exist = !!dataSource.find(n => n.id === current);
      if (exist) {
        return [...total, current];
      }
      return total;
    }, []);
    onModalConfirm(finalKeys);
  }, [onModalConfirm, keys, dataSource]);
