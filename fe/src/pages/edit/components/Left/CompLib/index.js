...
const CompLib = props => {
  const { addCusCompToUseCompList, compMenuList = [], getPluginMenu, compMenuListLoading } = props;
          ...
          return (
            <Col key={`${label}${compName}`} span={12} className={styles.silderItem}>
              <Card
                hoverable={true}
                onClick={() => onClick(v)}
                cover={<img alt={compName} src={src || getImgSrc()} />}
              >
...
CompLib.propTypes = {
  addCusCompToUseCompList: PropTypes.func,
};
