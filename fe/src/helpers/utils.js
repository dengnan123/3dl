...
export const getBasicStyle = props => {
...
export const getPreviewTransform = ({ left, top, PLeft, PTop, pageConfig }) => {
  if (isString(left)) {
    // 说明是vw
    return `translate(${parseFloat(left) - parseFloat(PLeft)}vw, ${top - PTop}px)`;
  }
...
export const getStyle = props => {
  const { left, top, PLeft, PTop, width, height, zIndex } = props;
  return {
    position: 'absolute',
...
export const dealWithCvData = ({ initUseCompList, id }) => {
  ...
  cvInfo.isLocking = 0;
  // 获取当前zindex的最高值
  cvInfo.zIndex = getMaxZIndex(initUseCompList) + 1;
  let newArr = [];
  let updateArr = [];
 ...
 * 获取所有子组件的ID
 */
export const getAllChildIds = child => {
  return child.reduce((pre, next) => {
    if (next.child) {
      return [...getAllChildIds(next.child), ...pre, next.id];
    }
    return [...pre, next.id];
  }, []);
};
