  ...
  return filter(
    map(schema, item => {
      if (includes(item.name.toLowerCase(), nameFilter)) {
        item = {
          ...item,
          columns: filter(item.columns, column => includes(column.toLowerCase(), columnFilter)),
        };
...
export default function SchemaBrowser({ schema, onRefresh, onItemSelect }) {
